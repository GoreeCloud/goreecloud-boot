...
| Mounted topology detection | Implemented, bounded | Intersects the discovered device topology with all major/minor identities in `/proc/self/mountinfo`; retains explicit root and boot evidence and rejects any discovered topology with a mounted filesystem. |
| Active swap topology detection | Implemented, bounded | Parses `/proc/swaps`; resolves swap partitions through sysfs and swap files through the deepest containing mountinfo filesystem; rejects a candidate when resolved active-swap identity intersects its discovered topology. Global swap evidence fails closed when unreadable, malformed, unsupported, or unresolvable. |
| Device revalidation token | Implemented, development-only | Compares a later discovered snapshot against selected current identity, removable/read-only state, topology, mounted-topology, and active-swap-topology evidence. Matching is necessary evidence only, not destructive authorization. |
...
| `bootctl list-linux-devices` | Implemented, development-only | Prints discovered Linux whole-device metadata, bounded recursive topology/mount/swap evidence, and current safety assessment. |
...
| Unit/synthetic-fixture tests | Implemented | Covers current catalog, target safety, Linux discovery/topology, mounted direct partitions, recursive holder relationships, active swap partitions/files/holder devices, fail-closed swap resolution, revalidation-token mount/swap changes, byte/sector layout, CRC32, protective MBR, GPT headers/entries, and 512/4096-byte logical-block cases. |

## Planned boot and provisioning features

| Feature | State | Notes |
| --- | --- | --- |
| Complete Linux destructive-target topology qualification | Planned | Must validate and, where necessary, extend conservative handling for device mapper, encrypted mappings, software RAID, multipath, hotplug/reconfiguration, namespaces, unusual swap/storage configurations, and other system-storage relationships before physical writes. Recursive `holders` traversal plus mounted-filesystem and active-swap exclusion are implemented layers, not complete qualification. |
