...
| Explicit device evidence model | Implemented | Retains the manual development evidence path for safety-policy testing, including general mounted-filesystem evidence. |
...
| Recursive Linux holder-topology discovery | Implemented, bounded | Starting from a whole device and directly enumerated partitions, follows sysfs `holders` links recursively upward and records discovered major/minor identities. This is not yet exhaustive destructive-target topology qualification. |
| Mounted topology detection | Implemented, bounded | Intersects the discovered device topology with all major/minor identities in `/proc/self/mountinfo`; retains explicit root and boot evidence and rejects any discovered topology with a mounted filesystem. Swap and topology cases not represented or validated by this model remain blockers. |
| Device revalidation token | Implemented, development-only | Compares a later discovered snapshot against selected current identity, removable/read-only state, topology, and mounted-topology evidence. Matching is necessary evidence only, not destructive authorization. |
| Conservative target-safety assessment | Implemented | Rejects non-removable, read-only, undersized, or filesystem-mounted target evidence, with specific root/boot reasons when applicable. This is not destructive-operation authorization. |
| Fail-closed per-device discovery | Implemented, development-only | A candidate with unreadable mandatory device or holder-topology evidence is omitted with a warning instead of being treated as eligible from incomplete evidence. |
...
| `bootctl plan-device` | Implemented, development-only | Evaluates explicit supplied evidence, including general mounted-filesystem evidence, and prints a proposed byte layout. Does not inspect or modify the device. |
| `bootctl list-linux-devices` | Implemented, development-only | Prints discovered Linux whole-device metadata, bounded recursive topology/mount evidence, and current safety assessment. |
...
| Unit/synthetic-fixture tests | Implemented | Covers current catalog, target safety, Linux discovery/topology, mounted direct partitions, recursive holder relationships, revalidation-token mount changes, byte/sector layout, CRC32, protective MBR, GPT headers/entries, and 512/4096-byte logical-block cases. |

## Planned boot and provisioning features

| Feature | State | Notes |
| --- | --- | --- |
| Complete Linux destructive-target topology qualification | Planned | Must validate and, where necessary, extend conservative handling for swap, device mapper, encrypted mappings, software RAID, multipath, hotplug, namespace, and other system-storage relationships before physical writes. Recursive `holders` traversal is only one implemented layer of this requirement. |
