...
- read-only Linux whole-block-device discovery from sysfs, caller-visible procfs mount-namespace/mount metadata, active-swap metadata, and available persistent aliases;
...
- directly enumerated child-partition identities plus recursive bidirectional sysfs `holders`/`slaves` traversal to build a bounded candidate topology;
- canonical-path cycle protection for reciprocal relation links and closure through shared holders to related backing members;
- discovery and deduplication of caller-visible mount namespaces by `/proc/<pid>/ns/mnt` identity;
- union of readable mountinfo evidence across distinct caller-visible namespaces, with explicit root/boot state retained and any mounted topology rejected;
- fail-conservative rejection of every candidate when caller-visible mount-namespace coverage is incomplete;
- active-swap discovery from `/proc/swaps`, including sysfs resolution of swap partitions and visible backing-filesystem resolution of swap files, with rejection when active swap intersects the discovered topology;
- fail-closed active-swap-file resolution when deepest backing mount evidence is ambiguous across readable visible mount namespaces;
- fail-closed omission of a candidate when mandatory per-device or required holder/slave-topology evidence cannot be read safely;
- fail-closed Linux discovery when mandatory global active-swap evidence cannot be read, parsed, or resolved safely;
- revalidation tokens that bind identity/geometry, removable/read-only state, discovered topology, mounted-topology intersection, active-swap-topology intersection, visible mount-namespace identities, and namespace-coverage state for later comparison;
...
- development `bootctl` inspection/planning/test-image commands, including display of visible mount-namespace identities and coverage completeness;
- unit and synthetic-fixture tests for those rules, including holder/slave topology cycles, mounted and active-swap sibling backing members, mounts present only in another visible namespace, incomplete visible namespace coverage, ambiguous swap-file backing across namespaces, and mount/swap/topology/namespace revalidation changes.
...
Current discovery reads standard Linux metadata sufficient to identify whole block devices, directly enumerated child partitions, a bounded bidirectional holder/slave topology, mounted filesystems visible through caller-accessible procfs mount namespaces, and current active swap areas. It must not treat a mutable path such as `/dev/sdb` as stable identity by itself.
...
- recursively discovered sysfs `holders` and `slaves` device identities;
...
- distinct caller-visible mount namespace identities from `/proc/<pid>/ns/mnt`;
- mounted major/minor identities and mount points unioned from distinct readable caller-visible `/proc/<pid>/mountinfo` views;
- explicit mounted root and boot associations across that visible evidence;
- whether caller-visible mount-namespace coverage was complete for the probe;
- active swap entries from `/proc/swaps`, resolved to major/minor identity either directly through sysfs for swap partitions or through deepest containing visible mount evidence for swap files.

A candidate topology starts with the whole device plus its direct partitions and recursively follows both `holders` and `slaves` links. Canonical-path deduplication prevents reciprocal link cycles. Traversal through a holder's slaves may add other backing members of that shared stacked device to the candidate topology. Discovery failure or missing mandatory per-device device/topology data must fail conservatively for the affected device rather than fabricating eligibility.

Mount evidence must not assume `/proc/self/mountinfo` is a system-wide mount inventory. Discovery must use the current mount namespace and other mount namespaces exposed through caller-visible numeric process entries under `/proc`, deduplicate by namespace identity, and union readable distinct mountinfo views. If a caller-visible process or distinct visible mount namespace cannot be safely inspected, the probe may still return metadata for read-only inspection but must mark mount-namespace coverage incomplete and make candidate assessment ineligible.

This visible namespace model is explicitly bounded by the caller’s procfs visibility and permissions. The implementation must not claim that hidden or inaccessible mount namespaces are absent merely because they cannot be observed.

Any mounted filesystem or resolved active swap area whose major/minor identity intersects the discovered topology makes the target ineligible. Global active-swap metadata is mandatory: if `/proc/swaps` cannot be read or an active entry cannot be parsed/resolved safely, Linux discovery must fail rather than return candidates based on incomplete host-storage evidence. If an active swap file maps to multiple distinct deepest backing devices across readable visible mount evidence, discovery must fail closed rather than select one arbitrarily.

This is a stronger development safeguard than self-namespace/root/boot-only matching, but it is not complete destructive-target qualification. Before any physical destructive write path can be enabled, Linux topology/state analysis must be extended or independently validated across relevant device-mapper, encrypted mapping, software RAID, multipath, hotplug/reconfiguration, procfs/PID/mount-namespace visibility constraints, unusual storage, and other relationships that could hide or introduce a system-disk dependency.
...
3. reject known mounted filesystems, root, boot, active swap, system-disk, incomplete namespace visibility, or contradictory storage topology;
4. reject read-only or undersized media;
5. show stable/current identity, geometry, topology, namespace/active-use evidence, and capacity to the user;
6. require explicit destructive authorization;
7. re-read and compare target identity and safety-relevant topology/active-use/namespace state immediately before writes;
8. abort on target disappearance, replacement, identity change, mount/swap/topology/namespace change, incomplete critical evidence, or contradictory evidence;
...
Core operation must remain local and offline-first. No image names, checksums, device identifiers, boot history, local configuration, or mount-namespace evidence may be transmitted merely to use the product. Current discovery and GPT planning are local-only but no substantive Privacy Shield integration contract is claimed yet.
...
- unit tests for safety, discovery/topology/mount-namespace/active-swap, catalog, layout, and GPT rules;
- synthetic Linux sysfs/procfs/mount/swap fixtures rather than reliance on CI-runner disk topology;
- recursive holder/slave chains, reciprocal cycles, mounted/active-swap sibling backing members, visible cross-namespace mounts, incomplete namespace coverage, ambiguous swap-file backing, topology/state/namespace changes, and fail-closed discovery cases;
- validation under representative procfs/PID/mount namespace visibility and privilege configurations before destructive use;
...
The current repository has advanced the safety foundation through read-only Linux discovery with bounded bidirectional holder/slave topology, caller-visible mount-namespace analysis with incomplete-coverage rejection, mounted-filesystem exclusion, active-swap exclusion, topology/active-use/namespace-aware revalidation evidence, sector-aware planning, and regular-file GPT metadata generation. Physical provisioning and boot execution remain outside the implemented boundary.